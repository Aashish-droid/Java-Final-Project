...
import com.dal.pharma.exceptions.CustomerInfoDoesNotExistException;
import com.dal.pharma.exceptions.MedInfoDoesNotExistException;
import com.dal.pharma.exceptions.SaleIdNotFoundException;
...
public class PharmaServiceImpl implements PharmaService {
	...
	@Override
    public MedInfo editMedicine(MedInfo medicine) throws MedInfoDoesNotExistException {
        if (medicine.getMedId() != 0 && medInfoRepo.existsById(medicine.getMedId())) {
            return medInfoRepo.save(medicine);
        } else {							// Handle the case when the appointment does not exist
            throw new MedInfoDoesNotExistException();
        }
	...
	@Override
	public CustomerInfo editCustomer(CustomerInfo customer) throws CustomerInfoDoesNotExistException {
		if (customer.getCustId() != 0 && medInfoRepo.existsById(customer.getCustId())) {
            return customerInfoRepo.save(customer);
        } else {							// Handle the case when the appointment does not exist
            throw new CustomerInfoDoesNotExistException();
        }
	}
	...
	@Override
	public SalesInfo findSale(long id) throws SaleIdNotFoundException {
	    Optional<SalesInfo> saleOptional = salesInfoRepo.findById(id);
